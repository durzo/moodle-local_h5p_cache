...
/**
 * The mod_hvp file storage class with cache support using muc
 *
...
namespace local_h5p_cache;
defined('MOODLE_INTERNAL') || die();

/**
 * The mod_hvp file storage class with cache support using muc.
 * Uses modified functions of the mod_hvp file_storage class copyrighted by
 * Joubal AS that is part of the mod_hvp plugin.
 *
 * @package    local_h5p_cache
 * @copyright  2020 Jordan Tomkinson <jordan.tomkinson@ethinkeducation.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 ...
 */
class file_storage extends \mod_hvp\file_storage {
    ...
    public function getTmpPath() {
        ...
        return make_request_directory() . '/' . uniqid('hvp-');
    }
        ...
        while (false !== ($file = readdir($dir))) {
            if (($file != '.') && ($file != '..') && $file != '.git' && $file != '.gitignore') {
                if (is_dir($source . DIRECTORY_SEPARATOR . $file)) {
                    ...
                    $fs = get_file_storage();
                    $newfile = $fs->create_file_from_pathname($record, $source . '/' . $file);

                    // Add the file to our cache
                    $cache = \cache::make('local_h5p_cache', 'filecache');
                    // Many H5P libraries contain the same files, so we check before adding
                    if (!$cache->has($newfile->get_contenthash())) {
                        //error_log("h5p_cache: adding to cache: " . $file . " contenthash: " . $newfile->get_contenthash());
                        $cache->set($newfile->get_contenthash(), file_get_contents($source . '/' . $file));
                    }
                }
            }
        }
    ...
    private static function exportFileTree($target, $contextid, $filearea, $filepath, $itemid = 0) {
        ...
        foreach ($files as $file) {
            ...
            if ($file->is_directory()) {
                ...
                $cache = \cache::make('local_h5p_cache', 'filecache');
                $cache_data = $cache->get($file->get_contenthash());
                // if the item is not in cache, download and store it.
                if (empty($cache_data)) {
                    //error_log("h5p_cache: Adding to cache: " . $file->get_filename());
                    // Copy file.
                    $file->copy_content_to($path . $file->get_filename());
                    // Add to the cache
                    $cache->set($file->get_contenthash(), file_get_contents($path . $file->get_filename()));
                }else {
                    // serve the file from our cache object
                    //error_log("h5p_cache: Fetching from cache: ". $file->get_filename());
                    file_put_contents($path . $file->get_filename(), $cache_data);
                }
            }
        }
    }
    ...
    private static function deleteFileTree($contextid, $filearea, $filepath, $itemid = 0) {
        ...
        if ($filepath === '/') {
            // get the contenthashes before we delete the files
            $sql = "SELECT DISTINCT(contenthash) FROM {files} WHERE filename != '.' AND component = 'mod_hvp' AND contextid = ? AND filearea = ? AND itemid = ?";
            $contenthashes = $DB->get_fieldset_sql($sql, [ $contextid, $filearea, $itemid ]);

            // Remove complete file area.
            $fs->delete_area_files($contextid, 'mod_hvp', $filearea, $itemid);

            // delete from cache if contenthash no longer exists
            foreach ($contenthashes as $contenthash) {
                if (!$DB->record_exists('files', array('contenthash' => $contenthash))) {
                    //error_log("h5p_cache: Deleting from cache: $contenthash");
        ...
        $files = $fs->get_directory_files($contextid, 'mod_hvp', $filearea, $itemid, $filepath, true);
        foreach ($files as $file) {
            // get the contenthash before deleting the file record.
